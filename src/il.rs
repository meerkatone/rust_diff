...
fn canonical_token(
    ...
    if kind.contains("symbol") || kind.contains("import") || kind.contains("indirect") {
        ...
        let resolved = rename_map.get(&tok.text).cloned().unwrap_or_else(|| tok.text.clone());
...
fn strip_dup_suffix(s: &str) -> &str {
    if let Some(pos) = s.rfind('_') {
        ...
        if !suffix.is_empty()
            ...
            && pos > 0 // keep at least one leading char
        {
            return &s[..pos];
        }
    }
...
pub fn diff_il(req: &IlDiffRequest) -> IlDiff {
    ...
    while i < n && j < m {
        if keys_a[i] == keys_b[j] {
            ...
            aligned.push(if same { Aligned::Equal(i, j) } else { Aligned::Rename(i, j) });
            i += 1;
            j += 1;
    ...
    let span_line = |idx_a: usize, idx_b: usize| {
        token_spans(&req.a.lines[idx_a], &req.b.lines[idx_b], &req.rename_map, &req.addr_map)
    };
    ...
    let denom = n.max(m);
    let similarity = if denom == 0 { 1.0 } else { lcs as f64 / denom as f64 };
...
fn token_spans(
    ...
    let ta: Vec<(&IlToken, String)> = line_a.tokens.iter()
        .map(|t| (t, canonical_token(t, rename_map, addr_map)))
        .collect();
    let tb: Vec<(&IlToken, String)> = line_b.tokens.iter()
        .map(|t| (t, canonical_token(t, rename_map, addr_map)))
        .collect();
    ...
    while i < na && j < nb {
        if ta[i].1 == tb[j].1 {
            sa.push(TokenSpan { text: ta[i].0.text.clone(), changed: false });
            sb.push(TokenSpan { text: tb[j].0.text.clone(), changed: false });
            i += 1;
            j += 1;
        } else if dp[i + 1][j] >= dp[i][j + 1] {
            // Don't emphasize whitespace/punctuation-only tokens (empty canonical).
            sa.push(TokenSpan { text: ta[i].0.text.clone(), changed: !ta[i].1.is_empty() });
            i += 1;
        } else {
            sb.push(TokenSpan { text: tb[j].0.text.clone(), changed: !tb[j].1.is_empty() });
            j += 1;
        }
    }
    while i < na {
...
fn pair_replacements(aligned: &mut Vec<Aligned>) {
    ...
    while k < aligned.len() {
        if matches!(aligned[k], Aligned::Delete(_)) {
            ...
            let dels: Vec<usize> = aligned[del_start..ins_start].iter()
                .map(|a| if let Aligned::Delete(i) = a { *i } else { unreachable!() }).collect();
            let inss: Vec<usize> = aligned[ins_start..k].iter()
                .map(|a| if let Aligned::Insert(j) = a { *j } else { unreachable!() }).collect();
...
mod tests {
    ...
    fn old_lines() -> Vec<IlLine> {
        vec![
            line(vec![tok("keyword", "if"), tok("text", "("), tok("keyword", "true"), tok("text", ")")], "if (true)"),
            line(vec![tok("keyword", "return"), tok("integer", "1")], "    return 1"),
            line(vec![tok("codeSymbol", "j__guard_check_icall"), tok("text", "("), tok("keyword", "nullptr"), tok("text", ")")], "j__guard_check_icall(nullptr)"),
            line(vec![tok("keyword", "return"), tok("codeSymbol", "testa"), tok("text", "("), tok("localVariable", "arg1"), tok("text", ", zx.q("), tok("localVariable", "arg2"), tok("text", "), "), tok("localVariable", "arg3"), tok("text", ")")], "return testa(arg1, zx.q(arg2), arg3)"),
        ]
    }
    // new (replace): if (false) ... return testb(...)
    fn new_lines_replace() -> Vec<IlLine> {
        vec![
            line(vec![tok("keyword", "if"), tok("text", "("), tok("keyword", "false"), tok("text", ")")], "if (false)"),
            line(vec![tok("keyword", "return"), tok("integer", "1")], "    return 1"),
            line(vec![tok("codeSymbol", "j__guard_check_icall"), tok("text", "("), tok("keyword", "nullptr"), tok("text", ")")], "j__guard_check_icall(nullptr)"),
            line(vec![tok("keyword", "return"), tok("codeSymbol", "testb"), tok("text", "("), tok("localVariable", "arg1"), tok("text", ", zx.q("), tok("localVariable", "arg2"), tok("text", "), "), tok("localVariable", "arg3"), tok("text", ")")], "return testb(arg1, zx.q(arg2), arg3)"),
        ]
    }
    ...
    fn replace_call_is_a_real_change() {
        ...
        let req = IlDiffRequest {
            a: IlFunction { level: "HLIL".into(), lines: old_lines() },
            b: IlFunction { level: "HLIL".into(), lines: new_lines_replace() },
        ...
        assert_eq!(replaces.len(), 2, "if-condition and call should both be replaces: {:#?}", d.lines);
    ...
    fn renamed_call_is_cosmetic_not_a_replace() {
        ...
        let call = d.lines.iter().find(|l| l.a.as_deref() == Some("return testa(arg1, zx.q(arg2), arg3)")).unwrap();
        assert_eq!(call.op, DiffOp::Rename, "matched-callee rename must be cosmetic: {:#?}", d.lines);
        // Exactly one real change remains: the if-condition flip.
        let replaces: Vec<&DiffLine> = d.lines.iter().filter(|l| l.op == DiffOp::Replace).collect();
        assert_eq!(replaces.len(), 1, "only the condition flip is a real change: {:#?}", d.lines);
    }
    ...
    fn identical_il_is_fully_equal() {
        let req = IlDiffRequest {
            a: IlFunction { level: "HLIL".into(), lines: old_lines() },
            b: IlFunction { level: "HLIL".into(), lines: old_lines() },
            rename_map: HashMap::new(),
            addr_map: HashMap::new(),
        };
        let d = diff_il(&req);
    ...
    fn ssa_dup_suffix_is_cosmetic() {
        ...
        let a = vec![line(vec![tok("keyword", "return"), tok("dataSymbol", "__xmm@aa0303")], "return __xmm@aa0303")];
        let b = vec![line(vec![tok("keyword", "return"), tok("dataSymbol", "__xmm@aa0303_1")], "return __xmm@aa0303_1")];
        let req = IlDiffRequest {
            a: IlFunction { level: "HLIL".into(), lines: a },
            b: IlFunction { level: "HLIL".into(), lines: b },
            rename_map: HashMap::new(),
            addr_map: HashMap::new(),
        };
        let d = diff_il(&req);
        assert_eq!(d.lines[0].op, DiffOp::Rename, "dup suffix must be cosmetic: {:#?}", d.lines);
    }

    #[test]
    fn long_numeric_suffix_is_preserved() {
        // `Feature_1196105017` is a real identifier, not a dup suffix.
        let a = vec![line(vec![tok("codeSymbol", "Feature_1196105017")], "Feature_1196105017")];
        ...
        assert_eq!(d.lines[0].op, DiffOp::Replace, "long id must not collapse: {:#?}", d.lines);
    }
    ...
    fn changed_code_address_is_a_real_change() {
        ...
        let a = vec![line(vec![tok("codeRelativeAddress", "0x1000"), tok("text", "(")], "0x1000(")];
        let b = vec![line(vec![tok("codeRelativeAddress", "0x2000"), tok("text", "(")], "0x2000(")];
        let req = IlDiffRequest {
            a: IlFunction { level: "HLIL".into(), lines: a },
            b: IlFunction { level: "HLIL".into(), lines: b },
            rename_map: HashMap::new(),
        ...
        assert_eq!(diff_il(&req).lines[0].op, DiffOp::Replace, "changed code address must surface");

        // Data/global addresses relocate; collapsed to ADDR -> cosmetic.
        let a = vec![line(vec![tok("keyword", "return"), tok("possibleAddress", "0x1801b8300")], "return 0x1801b8300")];
        let b = vec![line(vec![tok("keyword", "return"), tok("possibleAddress", "0x1801c9400")], "return 0x1801c9400")];
        let req = IlDiffRequest {
            a: IlFunction { level: "HLIL".into(), lines: a },
            b: IlFunction { level: "HLIL".into(), lines: b },
            rename_map: HashMap::new(),
        ...
        assert_eq!(diff_il(&req).lines[0].op, DiffOp::Rename, "relocated data address must be cosmetic");
    }
    ...
    fn relocated_matched_callee_is_cosmetic_via_addr_map() {
        ...
        assert_eq!(diff_il(&req).lines[0].op, DiffOp::Rename,
    ...
    fn replace_line_marks_only_changed_tokens() {
        ...
        let a = vec![line(vec![tok("keyword", "return"), tok("codeSymbol", "testa"),
                               tok("text", "("), tok("localVariable", "arg1"), tok("text", ")")],
                          "return testa(arg1)")];
        let b = vec![line(vec![tok("keyword", "return"), tok("codeSymbol", "testb"),
                               tok("text", "("), tok("localVariable", "arg1"), tok("text", ")")],
                          "return testb(arg1)")];
        let req = IlDiffRequest {
            a: IlFunction { level: "HLIL".into(), lines: a },
        ...
        let changed_a: Vec<&str> = l.a_spans.iter().filter(|s| s.changed).map(|s| s.text.as_str()).collect();
        let changed_b: Vec<&str> = l.b_spans.iter().filter(|s| s.changed).map(|s| s.text.as_str()).collect();
        assert_eq!(changed_a, vec!["testa"], "only the call symbol changed on A: {:#?}", l.a_spans);
        assert_eq!(changed_b, vec!["testb"], "only the call symbol changed on B: {:#?}", l.b_spans);
    }

    #[test]
    fn variable_renumbering_is_cosmetic() {
        let a = vec![line(vec![tok("keyword", "return"), tok("localVariable", "var_10")], "return var_10")];
        let b = vec![line(vec![tok("keyword", "return"), tok("localVariable", "var_18")], "return var_18")];
        let req = IlDiffRequest {
            a: IlFunction { level: "MLIL".into(), lines: a },
            b: IlFunction { level: "MLIL".into(), lines: b },
            rename_map: HashMap::new(),
            addr_map: HashMap::new(),
        };
        let d = diff_il(&req);
